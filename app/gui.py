...
def trigger_website_generation():
    ...
    if st.session_state.selected_mode == "LLM (Direct Website)":
        ...
        with st.status("🚀 Starting Direct Website Generation...", expanded=True) as status_ui:
            ...
            try:
                ...
                if "Error" not in html_output and "Input Error" not in html_output and "Processing Error" not in html_output:
                    status_ui.update(label="✅ Website generation complete!", state="complete")
                else:
                    status_ui.update(label="⚠️ Website generation finished with issues or input error.", state="error")
